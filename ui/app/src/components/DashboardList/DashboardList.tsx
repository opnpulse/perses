...
import { EphemeralDashboardInfo, DashboardResource } from '@perses-dev/client';
...
export function DashboardList(props: DashboardListProperties): ReactElement {
  ...
  const rows = useMemo(() => {
    ...
    // When the same dashboard name exists in several folders, prefix the display name with the
    // folder so the rows stay distinguishable in this flat list.
    ...
    return dashboardList.map<Row>((dashboard, index) => {
      const folder = dashboard.metadata.folderName ?? dashboard.metadata.folder;
      const name = dashboard.metadata.name;
      ...
      return {
        ...
        viewedAt: historyMap.get(`${dashboard.metadata.project}/${name}`),
      };
    });
            ...
            folders={[
              {
                kind: 'Folder',
                metadata: {
                  name: targetedDashboard.metadata.folderName ?? '',
                  project: targetedDashboard.metadata.project,
                },
                spec: {},
              },
            ]}
